...
export const HighlightSection: React.FC<{ highlights: any[] }> = ({highlights}) => (
    <div className="mt-12">
      ...
      <div className="grid gap-3">
        {highlights.length === 0 ? (
            <div
                className="p-16 text-center bg-[#1a1a1c] border border-gray-800 rounded-3xl text-gray-500 font-bold italic">진행된
              하이라이트가 없습니다.</div>
        ) : (
            [...highlights].reverse().map((hl) => (
                ...
                <span className="text-white font-mono text-xl font-black flex items-center gap-1.5 shrink-0">
                  <span
                      className="text-gray-500 text-lg">🎬</span> {formatOffset(hl.startTimeOffset)}
                </span>
                      <span
                          className="text-gray-200 text-[11px] font-bold">({formatAbsoluteTime(hl.startTime)})</span>

                      <span className="text-gray-200 font-black text-lg px-2 shrink-0">~</span>

                      {hl.status === 'ONGOING' ? (
                          <div
                              className="flex items-center gap-1.5 px-2 py-1 bg-red-500/10 rounded-lg border border-red-500/20">
