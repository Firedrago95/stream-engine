...
export const AnalysisChart: React.FC<Props> = ({
  ...
  return (
      <div
          className="p-4 sm:p-8 bg-[#0c0d0f] border border-gray-800 rounded-3xl relative overflow-hidden min-h-[500px]">
        ...
        {/* 로딩 중/수집 중 오버레이 */}
        {isRealtime && (isLoading || isGathering || chartData.filter(d => d.hasData).length === 0) && !error && (
            <div
                className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm">
              <div
                  className="w-10 h-10 border-4 border-[#00FFA3] border-t-transparent rounded-full animate-spin mb-4"/>
              <p className="text-[#00FFA3] font-black tracking-widest text-sm uppercase">Engine
                Analyzing...</p>
            </div>
        )}
        ...
        <div className="flex justify-between items-start mb-10">
          <h3 className="text-lg font-bold text-gray-400 uppercase tracking-widest italic">Chat
            Intensity</h3>
          <div className="text-right">
            <span
                className="text-[10px] text-gray-500 font-bold block mb-1 uppercase tracking-tighter">{metric.label}</span>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis
                  ...
                  minTickGap={isRealtime ? 50 : 80} // 글자 겹침 방지
                  stroke="#475569"
                  fontSize={10}
                  tickMargin={15}
                  axisLine={false}
              ...
              <Tooltip
                  content={<CustomTooltip selectedTab={selectedTab} formatTime={formatTime}/>}
