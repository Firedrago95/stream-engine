...
class RedisChatRoomAggregationRepositoryTest implements RedisTestSupport {
    ...
    void 채팅갯수를_저장한다() {
        ...
        List<List<Object>> result = redisTemplate.execute(tsRangeScript, List.of(key), "-", "+");

        assertThat(result).isNotNull().hasSize(1);
        long savedTimestamp = ((Number) result.getFirst().get(0)).longValue();
        long savedCount = Long.parseLong((String) result.getFirst().get(1));
    ...
    void 채팅방별_채팅집계_기록을_조회한다() {
        ...
        assertAll(
            ...
            () -> assertThat(dataPoints.getFirst().value()).isEqualTo(1),
