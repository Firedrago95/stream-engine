...
public class TargetStreamPool {
    ...
    public void syncTargets(Set<String> newTargets) {
        ...
        try {
            ...
            redisTemplate.opsForSet().add(key, newTargets.toArray(new String[0]));
            if (!customChannels.isEmpty()) {
                redisTemplate.opsForSet().add(key, customChannels.toArray(new String[0]));
            }
            log.info("[Targeting] 타겟 스트리머 명단이 동기화되었습니다. (총 {}개 채널)", newTargets.size() + customChannels.size());
        } catch (Exception e) {
